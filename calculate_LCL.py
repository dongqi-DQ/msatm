...
def calculate_LCL(T,r,p,c=load_constants('default')):
    """
    
     Function to calculate the lifted condensation level (LCL) for a parcel.
    
    
     [T_LCL,p_LCL,[T_LCLl,p_LCLl,T_LCLi,p_LCLi]] = calculate_LCL(T,r,p,c=c)
    ...
    T_LCLl = cc*(lambertw(RHl**(1/ac)*cc*np.exp(cc),-1).real)**(-1)*T
    ...
    T_LCLi = cc*(lambertw(RHi**(1/ac)*cc*np.exp(cc),-1).real)**(-1)*T
    ...
    if c.ice > 0:
        ...
        while (max(abs(dT))>0.02) & (I.sum()>0):
            ...
            T_LCL_new = cc[I]*(lambertw(lambert_input,-1).real)**(-1)*T[I]
